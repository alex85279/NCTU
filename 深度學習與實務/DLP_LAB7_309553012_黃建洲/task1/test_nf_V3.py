...
class Glow(nn.Module):
    ...
    def __init__(self, num_channels, num_levels, num_steps, mode='sketch'):
        ...
        self.flows = _Glow(in_channels=4 * 3,  # RGB image after squeeze
                           ...
                           num_steps=num_steps)
        self.mode = mode
        self.condToInputSize = nn.Linear(24,64*64,bias=False)
    def forward(self, x, x_cond, reverse=False):
        x_cond = self.condToInputSize(x_cond).view(-1,1,64,64)
        if reverse:
...
def interpolation_image(model, train_dataset, idx1, idx2, device, epoch):
    ...
    for i in range(5):
        ...
        image1 = image1.to(device).view(1,3,64,64)
        image2 = image2.to(device).view(1,3,64,64)
        ...
        for i in range(8):
            ...
            output_image = torch.cat([output_image, fake_image.view(1,3,64,64).cpu()],0)
    save_image(make_grid(output_image, nrow=8),"./figure/"+ 'interpolate_'+ str(epoch) + ".jpg")
def attribute_interpolation(model, train_dataset, idx, device, epoch):
    output_image = torch.randn(0, 3, 64, 64)
    for i in range(5):
        i1 = idx[i]
        output_image = torch.randn(0, 3, 64, 64)
        image, condition = train_dataset[i1]
        image = image.to(device).view(1,3,64,64)
        condition = torch.from_numpy(condition)
        condition = condition.to(device).type(torch.float32).view(1,40)
        neg_condition = torch.clone(condition)
        neg_condition[0,31] = -1
        pos_condition = torch.clone(condition)
        pos_condition[0,31] = 1
        
        z_neg,_ = model(image, neg_condition, reverse=False)
        z_pos,_ = model(image, pos_condition, reverse=False)
        z_inv = (z_pos - z_neg) / 8
...
def runModel(batch_size, image_size, condition_size, noise_size, epochs, device, train_data, test_data, model, optimizer, evaluation_model, train_dataset, test_dataset):
    total_acc = []
    loss_all = []
    iter_i = 0
        ...
        for x, real_condition in train_data:
            ...
            if (iter_i - 1) % 50 == 0:
                ...
                for real_condition in test_data:
                    ...
                    ##############
                    acc = evaluation_model.eval(output_image, real_condition)
                    print("acc: ", acc)
...
if __name__ == '__main__':
    ...
    train_dataset = dataset.ICLEVRLoader('./', mode = 'train')
    test_dataset = dataset.ICLEVRLoader('./', mode = 'test')
    ...
    model = Glow(64, 3, 16, "sketch").to(device)
    
    optimizer = optim.Adam(model.parameters(), lr=LR_G, betas=(0.5, 0.999))
    
    criterion_BCE = nn.BCELoss()
    evaluation_model = evaluator.evaluation_model()
    #runModel(batch_size, image_size, noise_size, condition_size, epochs, device, train_data, test_data, model, optimizer, evaluation_model, criterion):
    runModel(batch_size, image_size, condition_size, noise_size, epochs, device, train_data, test_data, model, optimizer, evaluation_model, train_dataset, test_dataset)