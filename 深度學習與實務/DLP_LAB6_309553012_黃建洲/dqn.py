...
class DQN:
    ...
    def select_action(self, state, epsilon, action_space):
        ...
        if random.random() > epsilon:
            ...
            with torch.no_grad():
                action_qv = self._behavior_net(s)
            self._behavior_net.train()
            return np.argmax(action_qv.cpu().data.numpy())
    ...
    def update(self, total_steps):
        if total_steps % self.freq == 0:
            ...
            #self._DDQN_update_behavior_network(self.gamma)
    ...
    def _DDQN_update_behavior_network(self,gamma):
        state, action, reward, next_state, done = self._memory.sample(
            self.batch_size, self.device)
        q_value = self._behavior_net(state).gather(1, action.long())
        with torch.no_grad():
            q_max = self._behavior_net(next_state).detach().max(1)[1].unsqueeze(1)
            q_next = self._target_net(next_state).detach().gather(1, q_max)
            q_target = reward + gamma * q_next * (1 - done)
        criterion = nn.MSELoss()
        loss = criterion(q_value, q_target)
        # optimize
...
def main():
    ...
    parser.add_argument('--logdir', default='log/dqn_normal')
    # train
